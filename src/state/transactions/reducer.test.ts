...
describe('transaction reducer', () => {
  ...
  describe('addTransaction', () => {
    it('adds the transaction', () => {
      ...
      store.dispatch(
        addTransaction({
          chainId: ChainId.MAINNET_CHILD_0,
          summary: 'hello world',
          hash: '0x0',
      ...
      expect(txs[ChainId.MAINNET_CHILD_0]).toBeTruthy()
      expect(txs[ChainId.MAINNET_CHILD_0]?.['0x0']).toBeTruthy()
      const tx = txs[ChainId.MAINNET_CHILD_0]?.['0x0']
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.MAINNET_CHILD_0), String(ChainId.RINKEBY)])
      expect(Object.keys(store.getState()[ChainId.MAINNET_CHILD_0] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.RINKEBY] ?? {})).toEqual(['0x1'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.MAINNET_CHILD_0 }))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.MAINNET_CHILD_0), String(ChainId.RINKEBY)])
      expect(Object.keys(store.getState()[ChainId.MAINNET_CHILD_0] ?? {})).toEqual([])
      expect(Object.keys(store.getState()[ChainId.RINKEBY] ?? {})).toEqual(['0x1'])
    })
  })
})
