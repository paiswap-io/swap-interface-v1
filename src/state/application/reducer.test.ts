...
describe('application reducer', () => {
  ...
  describe('updateBlockNumber', () => {
    it('updates block number', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.MAINNET_CHILD_0, blockNumber: 4 }))
      expect(store.getState().blockNumber[ChainId.MAINNET_CHILD_0]).toEqual(4)
    })
    it('no op if late', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.MAINNET_CHILD_0, blockNumber: 2 }))
      expect(store.getState().blockNumber[ChainId.MAINNET_CHILD_0]).toEqual(3)
    })
